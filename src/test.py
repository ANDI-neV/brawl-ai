...
def check_brawler_significance():
    brawler_significance = {}
    db = Database()
    brawler_data = prepare_brawler_data()
    for brawler in brawler_data.keys():
        brawler_significance[brawler] = db.checkBrawlerSignificance(brawler = brawler.upper(), map='Out in the Open')

    print("Brawler significance:")
    for brawler, significance in brawler_significance.items():
        print(f"{brawler}: {significance}")

if __name__ == '__main__':
    db = Database()
    check_brawler_significance()